...
# Install Zen Browser
if ($Zen) {
	Write-Output "Downloading Zen..."
	& curl.exe -LSs "https://github.com/zen-browser/desktop/releases/latest/download/zen.installer.exe" -o "$tempDir\zen.exe"
	Start-Process -FilePath "$tempDir\zen.exe" -WindowStyle Hidden -ArgumentList '/S /ALLUSERS=1' -Wait 2>&1 | Out-Null
	exit
}
